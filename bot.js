...
bot.on('callback_query', (callbackQuery) => {
  ...
  if (callbackQuery.data === 'kalori') {
    ...
    function defuzzyfikasi() {
      ...
      } else if (jenisKelaminBaru == 'Perempuan') {
        if (umur >= 0 && umur <= 3) {
    ...
    bot.sendMessage(message.chat.id, `Kebutuhan kalori kamu adalah : ` + hasil());
...
bot.onText(/\/settings/, (msg) => {
  ...
  const chatId = msg.chat.id;
  // menampilkan pilihan untuk mengisi data
  bot.sendMessage(
    chatId,
    'Berikut data yang kami perlukan',
    {
      reply_markup: {
        inline_keyboard: [[
          {
            text: 'Umur',
            callback_data: '/setumur'
          }, {
            text: 'Tinggi Badan',
            callback_data: '/setTB'
          }, {
...
// Listener (handler) for callback data from /settings command
bot.on('callback_query', (callbackQuery) => {
  const message = callbackQuery.message;
  const setting = callbackQuery.data;
  if (setting === '/setumur') {
    bot.sendMessage(message.chat.id, "Masukan umur kamu").then(function () {
      answerCallbacks[message.chat.id] = function (answer) {
        umur = answer.text;
        bot.sendMessage(message.chat.id, 'Umur ' + umur + ' tahun berhasil disimpan!')
      }
    });
  } else if (setting === '/setTB') {
    bot.sendMessage(message.chat.id, "Masukan tinggi badan kamu").then(function () {
      answerCallbacks[message.chat.id] = function (answer) {
        tinggiBadan = answer.text;
        bot.sendMessage(message.chat.id, 'Tinggi badan ' + tinggiBadan + ' cm berhasil disimpan!')
      }
    });
  } else if (setting === '/setBB') {
    bot.sendMessage(message.chat.id, "Masukan berat badan kamu").then(function () {
      answerCallbacks[message.chat.id] = function (answer) {
        beratBadan = answer.text;
      ...
      'Pilih gender kamu',
      {
        reply_markup: {
          inline_keyboard: [[
            {
              text: 'Laki-laki',
              callback_data: 'Laki-laki'
            }, {
              text: 'Perempuan',
              callback_data: 'Perempuan'
            }
          ]]
        }
      }
      ...
      'Pilih jenis aktifitas kamu',
      {
        reply_markup: {
          inline_keyboard: [[
            {
              text: 'Ringan',
              callback_data: 'ringan'
            }, {
              text: 'Sangat Ringan',
              callback_data: 'sangatringan'
...
// Listener (handler) for callback data from setgender command
bot.on('callback_query', (callbackQuery) => {
  const message = callbackQuery.message;
  if (callbackQuery.data === 'Laki-laki') {
    gender = callbackQuery.data;

    bot.sendMessage(message.chat.id, `Gender laki-laki`);
  } else if (callbackQuery.data === 'Perempuan') {
    gender = callbackQuery.data;
...
bot.onText(/\/help/, (msg) => {
  ...
  bot.sendMessage(chatId, `Hai! Saya @mykaloribot, saya dirancang untuk menghitung kebutuhan kalori kamu sepenuhnya menggunakan algoritma *fuzzy logic*.
  
Untuk mulai menggunakan bot ini kamu diwajibkan mengisi data diri kamu pada /settings

Setelah itu kamu dapat melihat kebutuhan kalori kamu pada menu /profile dan pilih cek kalori.

[/start](/start)        - Untuk memulai bot.
[/profile](/profile)    - Melihat kebutuhan kalori kamu.
[/settings](/settings)  - Data diri kamu.
